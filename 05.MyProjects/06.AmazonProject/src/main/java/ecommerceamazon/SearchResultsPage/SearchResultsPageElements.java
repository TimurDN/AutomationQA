...
public class SearchResultsPageElements extends BasePageAmazon {
    ...
    public WebElement searchedProductInResults(String productName) {
        this.actions.waitTillElementIsVisible(String.format("(//span[contains(text(), '%s')])[3]", productName));
        return this.driver.findElement(By.xpath(String.format("(//span[contains(text(), '%s')])[3]", productName)));
    }
}